...
# script to create Glicko2 prediction model
"""
Predicts match outcomes using Glicko-2 ratings with uncertainty
Uses weekly Glicko-2 snapshots
...
class Glicko2Model:
    def __init__(self, variant=""):
        # init Glicko2 prediction model
        # variant: node suffix for diff model variants (e.g., "_stable", "_volatile")
        ...
        if variant: print(f"Using variant: {variant}")
        print("=" * 60)
    ...
    def check_date_validity(self, match_date, force=False):
        # check if date is valid for Glicko2 preds
        # returns: true if valid, false if should abort
        if match_date < GLICKO2_MINIMUM_DATE:
            print(f"\nERROR: Cannot make predictions before {GLICKO2_MINIMUM_DATE}")
        ...
        if match_date < GLICKO2_WARNING_DATE and not force:
            print(f"\nWARNING: Match date {match_date} is very early in Glicko-2 history")
    ...
    def calculate_glicko2_probability(self, rating_a, rd_a, rating_b, rd_b):
        # calc win prob using Glicko2 formula > accounts for rating deviation (uncertainty)
        """
    ...
    def get_surface_ratings(self, glicko2_data, surface_type):
        ...
        if not glicko2_data: return None, None, None
        ...
        # for surface preds, blend overall and surface-specific
    ...
    def predict_match(self, player1_name, player2_name, match_date, surface='overall', verbose=True, return_ratings=False, force=False):
        # predict match outcome w/ Glicko2 ratings at specified date
        # check date validity
        ...
        if verbose:
            ...
            if self.variant: print(f"Model variant: {self.variant}")
            print("-" * 50)
        ...
        if verbose: print(f"Found: {p1_full_name} vs {p2_full_name}")
        
        # get Glicko2 ratings at match date
    ...
    def batch_evaluate(self, year, surface='overall', tournament=None, round=None, output_file=None):
        # batch eval Glicko2 model w/ specified filters
        """
        Args:
        ...
        if results:
            ...
            if not output_file:
                output_file = self.generate_output_filename(year=year, surface=surface, tournament=tournament, round=round)
            
            # ensure output file is in model_outputs directory
            if not output_file.startswith(self.output_dir): output_file = os.path.join(self.output_dir, os.path.basename(output_file))
    ...
    def generate_output_filename(self, year, surface='overall', tournament=None, round=None):
        ...
        # combine + add extension
        filename = '_'.join(parts) + '.csv'
        
        return os.path.join(self.output_dir, filename)
    
    def interactive_mode(self):
        # interactive mode for testing preds
        ...
        while True:
            try:
                ...
                if player1.lower() == 'quit': break
                
                player2 = input("Enter Player 2 name: ").strip()
                if player2.lower() == 'quit': break
                ...
                try: match_date = int(match_date_input)
                except ValueError:
                ...
                if not surface: surface = 'overall'
